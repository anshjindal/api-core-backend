...
try {
  addressData = JSON.parse(
    fs.readFileSync("tests/WOUESSITESTDATA/Wouessi.address.json")
  );
  departmentData = JSON.parse(
    fs.readFileSync("tests/WOUESSITESTDATA/Wouessi.departments.json")
  );
  designationData = JSON.parse(
    fs.readFileSync("tests/WOUESSITESTDATA/Wouessi.designations.json")
  );
  employeeData = JSON.parse(
    fs.readFileSync("tests/WOUESSITESTDATA/Wouessi.employees.json")
  );
  roleData = JSON.parse(
    fs.readFileSync("tests/WOUESSITESTDATA/Wouessi.roles.json")
  );
} catch (err) {
...
if (require.main === module) {
  runTests().finally(() => mongoose.connection.close());
}
